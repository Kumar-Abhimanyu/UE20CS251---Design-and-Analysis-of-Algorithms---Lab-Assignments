...
static void q1_helper(int x, int vis[], int n, const connection_t  graph[n][n]){
    vis[x] = 1;
    for(int i = x; i<n; i++){
        for(int j = 0; j<n; j++){
            if(graph[i][j].distance != __INT_MAX__ && vis[j]!=1){
                q1_helper(j, vis, n, graph);
            }
        }
    }
}
...
static void q2_helper(int x, int vis[], int n, const connection_t graph[n][n], int dest, int num, int *ans){
    vis[x] = 1;
    if(x==dest){
        int flag = 0;
        for(int i = 0; i<n; i++){
    ...
    for(int i = x ; i < n ; i++)
    {
        for(int j = 0 ; j < n ; j++)
        {
            if (graph[i][j].distance != __INT_MAX__ && vis[j]!= 1)
            {
               q2_helper(j , vis , n , graph , dest , num , ans);
            }
        }
    }
}
...
static void q3_helper (int x , int vis[] , int n , const connection_t matrix[n][n] , int s , int *ans)
{
    vis[x] = 1;
    for(int i = x ; i < n ; i++)
    {
        for(int j = 0 ; j < n ; j++)
        {
            ...
            {
                q3_helper(j , vis , n , matrix , s , ans);
            } 
        }
    }
}
...
int q1(int n, const connection_t connections[n][n])
{
    ...
    for(int i = 0; i<n; i++){
        q1_helper(i,visited,n,connections);
       ...
       const connection_t connections[n][n])
{
    ...
    q2_dfs(start_val , vis , n , connections , dest_val , k , &ans);
    if(ans == 1)
    {
        return 1;
    }
...
int q3(const airport_t *src, int n, const connection_t connections[n][n])
{
    ...
    q3_helper(start , vis , n , connections, start , &ans);
    return ans;
}
...
{
    int start=0, ender=n-1;
    int mid;
    while(start <= ender) {
        mid = start + (ender-start)/2; 
